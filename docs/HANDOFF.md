...
15. AI 격차 완화 및 랭킹 UI 회귀 방지 보강:
   - AI 획득 배수를 플레이어와 동일(`1.82`)로 동기화
   - AI 속도 배수 상향(`0.88 -> 0.96`) 및 순위 기반 `catchupBonus` 복원
   - 랭킹 패널 배치 기준을 뷰포트가 아닌 게임 뷰 좌측 상단 앵커로 고정
   - 랭킹 패널은 화면비 변경 시 비례 스케일 유지 + 게임 뷰 경계 클램프 적용
   - 랭킹 목록에서 `YOU` 항목은 노란색 강조로 표기
